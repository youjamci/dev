			...
			<form action="/notice/signup.do" method="post" id="signupForm" enctype="multipart/form-data">
				...
				<div class="input-group mb-3">
					...
					<span class="input-group-append">
						<button type="button" class="btn btn-secondary btn-flat" id="idCheckBtn" name="idCheckBtn">중복확인</button>
					</span>
...
$(function(){
	...
	idCheckBtn.on("click", function(){
		...
		// 위와 같은 설정으로 진행하면, request header 정보에 contentType은 json으로 넘어가고, payload는 json 객체 형식으로 데어터가 넘어감
		var data = {
				memId : id
		};
...
// 			data : {memId : id}, // 방법 1) contentType 설정 안함 / data도 {id:1}과 같이 설정 /// 얘만 설정해서 보냄
	...
	signupBtn.on("click", function(){
		...
		// 개인정보 처리 방침을 동의하게 되면 Y값이 넘어오므로, 동의 여부는 true로설정
		...
		// 개인정보 처리 방침을 등의하셨습니까?
		// 아이디 중복 체크를 하고 오셨습니까?
		// 오셨다구요? 그럼 가입하기를 진행할께요
		// 예? 안했다구요? 그럼 가입하기를 할 수 없습니다.
		if(agreeFalg){ // 개인정보처리방침 OK
