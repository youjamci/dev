...
public class NoticeDownloadController {
	...
	public View noticeProcess(int fileNo, ModelMap model) { /// model과 modelmap은 똑같음 - 같은 메소드를 제공
		...
		// 리턴되는 noticeDownloadView는 jsp페이지로 존재하는 페이지 Name을 요청하는 게 아니라, 클래스를 요청하는 것인데 해당 클래스가 스프링에서 제공하는 AbstractView 클래스를 상속받은 클래스인데
		// 그 클래스는 AbstractView를 상속받아 renderMergedOutputModel 함수를 재정의할 때 View로 취급될 수 있게 해준다.
		return new NoticeDownloadView(); /// 얘가 페이지처럼 만들어져서 리턴하게끔 만들꺼임
	}
}
