...
public class uploadFileUtils {
	...
	private static String calcPath(String uploadPath) {
		...
		// DecimalFormat("00") :: 두 자리에서 빈 자리는 0으로 채움
	...
	private static void makeThumbnail(String uploadPath, String path, String fileName) throws IOException {
		...
		/// 파일객체를 생성하고 아래 write를 통해 만듦
		File newFile = new File(thumbnailName);
		String formatName = fileName.substring(fileName.lastIndexOf(".") + 1);
		
		// "s_"가 붙은 썸네일 이미지를 만든다.
		ImageIO.write(destImg, formatName.toUpperCase(), newFile); /// formatName.toUpperCase() - 확장자
	}
}
