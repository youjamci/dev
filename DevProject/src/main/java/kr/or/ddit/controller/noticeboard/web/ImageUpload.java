...
public class ImageUpload {
	...
	public String imageUpload(
		...
		JsonObject json = new JsonObject(); // JSON 객체를 만들기 위한 준지
		PrintWriter printWriter = null; // 외부 응답으로 내보낼 떄 사용할 객체
			...
			if(file.getContentType().toLowerCase().startsWith("image/")) {
				...
				if(file.getSize() > limitSize) { // 최대 크기 2MB 보다 클 때
					...
					try {
						...
						// 업로드 경로로 설정된 폴더구조가 존재하지 않는 경우, 파일을 복사할 수 없으므로 폴더 구조가 존재하지 않는 경우 생성하고 존재하는 경우 건너뛰로록 한다.
						File uploadFile = new File(uploadPath);
						if(!uploadFile.exists()) {
							uploadFile.mkdirs();
						}
						fileName = UUID.randomUUID().toString(); // UUID의 random 파일명을 생성한다.
						uploadPath = uploadPath + "/" + fileName + ".jpg"; // 업로드 경로 + 파일명.jsp
						out = new FileOutputStream(new File(uploadPath));
						out.write(bytes); // 파일 복사
						
						printWriter = resp.getWriter(); /// 응답으로 내보낼 준비
						
						/*
						 * {
						 * 		"uploaded" : 1,
						 * 		"fileName" : "xxxxxxxx-xxxxxxx.jpg",
						 * 		"url" : "/resources/img/xxxxxxxx-xxxxxxx.jpg"
						 * }
						 */
