...
public class FileUploadController02 {
	 ...
	 * 			- 파일 업로드 목록 화면 만들기 (item2/list.jsp_
	 * 			-		여기까지 확인
	 * 
	 ...
	 * 			-		여기까지 확인
	 * 
	 * 			- 파일 업로드 삭제  기능 컨트롤러 메소드 만들기(item2Modify:post)
	 * 			- 파일 업로드 삭제  기능 서비스 인터페이스 메소드 만들기
	 * 			- 파일 업로드 삭제 기능 서비스 클래스 메소드 만들기
	 * 			- 파일 업로드  기능 Mapper 인터페이스 메소드 만들기
	 * 			- 파일 업로드  기능 Mapper xml 쿼리 만들기
	 * 			-		여기까지 확인
	 * 
	 * 			- 파일 업로드 삭제  기능 컨트롤러 메소드 만들기(item2Modify:post)
	 * 			- 파일 업로드 삭제  기능 서비스 인터페이스 메소드 만들기
...
//	@GetMapping(value = "/register") // 아래와 같은 요청 매핑
	...
	public String item2Register(Item2 item, Model model) throws IOException {
		...
		for(int i = 0; i < pictures.size(); i++) {
			...
			log.info("originalName : " + file.getSize());
			log.info("size : " + file.getSize());
			log.info("contentType : " + file.getContentType());
			
 			String savedName = uploadFile(file.getOriginalFilename(), file.getBytes());
 			
 			if(i == 0) { // 첫 번째 URL 정보 셋팅
 				item.setPictureUrl(savedName);
 			} else if(i == 1) { // 두 번째 URL 정보 셋팅
