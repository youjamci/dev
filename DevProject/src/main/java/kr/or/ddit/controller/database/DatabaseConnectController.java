...
public class DatabaseConnectController {
	 ...
	 * 					title varcahr2(200) not null,
	 * 					content varchar2(2000) null,
	 * 					writer vatchar2(50) not null,
	 ...
	 * 			JDBC의 방식은 기존 PrepareStatement를 활용한 방식과 동일하므로 넘어가도록 합시다.
	 * 
	 * 6. JPA
	 ...
	 * 		# JPA 관련 어노테이션
	 * 
	 * 			자바빈즈 클래스 객체가 곧 테이블 구조와 같은 형태이기 때문에, VO클래스 내 @Entity 어노테이션을 통해 해당 객체가 테이블과 같은 Entity임을 설정하고
	 ...
	 * 			설정된 Entity들을 이용하려 jpa 객체를 통해서 함수를 호출하면(persist, find 등등) 호출된 함수를 통해 Spring Data JPA가 
	 ...
	 * 			그러기 위해선 관련 의존관계를 등록 후 사용해야합니다. (관련 라이브러리는 hibernaete 말고도 설정에 따라 다양할 수 있음)
	 * 			SQL문을 이용하여 데이터베이스를 연동하여 데이터를 가공하는게 아닌, Method를 통해 데이터베이스를 조작할 수 있다는 장점이 있어
	 * 			객체모델을 이용하여 비지니스 로직을 구성하는데에만 집중을 할 수 있다.
	 * 			
	 * 			하지만, 프로젝트의 규모와 복잡도가 클 때는 설계가 잘못되는 경우 해당 프로젝트의 품질이 떨어진다.
	 ...
	 * 			[용어]
	 * 		
	 * 				** ORM이란?[면접 단골 질문 - ORM용어, ORM 어떤거 썼는지]
	 ...
	 *				> mapper 패키지 안에 BoardMapper.xml 구성 후 사용 준비
	 */
}
