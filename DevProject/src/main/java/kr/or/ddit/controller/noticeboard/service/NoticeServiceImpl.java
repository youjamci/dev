...
//			try {
//				tst.sendGet("이나은", noticeVO.getBoTitle());
//			} catch (IOException e) {
//				e.printStackTrace();
//			}
			result = ServiceResult.OK;
	...
	public ServiceResult updateNotice(HttpServletRequest req, NoticeVO noticeVO) {
		...
		if(status > 0){
			...
			try {
				// 공지사항 파일 업로드 
				noticeFileUpload(noticeFileList, noticeVO.getBoNo(), req); /// 여길 통해 업로드
				
				// 기존에 등록되어 있는 파일 목록들 중, 수정하기 위해서 x버튼을 눌러 삭제 처리로 넘겨준 파일 번호들 /// 여길 통해 삭제
		...
		if(status > 0){
			...
			try {
				if(noticeFileList != null && noticeFileList.size() > 0) {
					// [0] = D:\99.Class\02.SPRING@\workspace_spring2\.metadata\...\
		...
		try {
			if(folder.exists()) { // 경로가 존재한다면
				...
				for(int i = 0; i < folderList.length; i++) {
					if(folderList[i].isFile()) { // 폴더 안의 있는 파일이 파일일 때
						folderList[i].delete(); // 폴더 안에 파일을 차례대로 삭제
					}else { /// 파일이 아니라 폴더일 때
						// 폴더 안의 있는 파일이 폴더일 때 재귀함수 호출 (폴더 안으로 들어가서 다시 이행)
