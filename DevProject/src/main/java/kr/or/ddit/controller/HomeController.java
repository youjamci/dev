...
public class HomeController {
	...
	// 반환값이  "redirect:"로 시작하면 라다이렉트 방식으로 처리한다.
	/// /sub/goHome0204일로 주소 요청하면 /sub/goHome0203여기로 이동함
	 ...
	 * 
	 * @ResponseBody를 지정하지 않으면 HTTP 404에러가 발생한다.(jackson-databind 라이브러리가 설정되어 있지 않는 경우)
	 ...
	 */
	// 200 OK 상태 코드를 전송한다.
	// Void 타입은 아무런 형태가 아닌데 제네릭타입의 뭔가는 채워야겠어서 채우는 Placeholder와 같은 느낌이랄까?(ResponseEntity<Void>안에 보이드는 아무 의미 없음)
	@ResponseBody
	...
	public ResponseEntity<byte[]> goHome1101(){
		...
		try {
			...
			in = new FileInputStream("D:\\99.JSP&SPRING\\02.SPRING2\\workspace_spring2\\image\\다운로드.jpg");
	...
	public ResponseEntity<byte[]> goHome1102(){
		...
		String fileName = "다운로드.jpg";
		InputStream in = null;
		ResponseEntity<byte[]> entity = null;
		
		try {
			HttpHeaders header = new HttpHeaders();
			in = new FileInputStream("D:\\99.JSP&SPRING\\02.SPRING2\\workspace_spring2\\image\\" + fileName);
