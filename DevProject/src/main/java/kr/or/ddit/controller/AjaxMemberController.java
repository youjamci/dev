...
public class AjaxMemberController {
	...
	// 1) UTL 경로 상의 어려 개의 경로 변수값을 @PathVariable 어노테이션을 지정하여 여러 개의 문자열 매개변수로 처리한다.
	...
	// 2) 객체 타입의 JSON 요청 데이터를 @RequestBody 어노테이션을 지정해 자바빈즈 매개변수로 처리한다.
	// @RequestBody 안 붙이면 vo에 저장된 대로 나옴
	...
	public ResponseEntity<String> register03(String userId){
		// 요청 본문에서 데이터가 바인됭 되지 않아 userId가 null이 나온다.
	...
	// 6) 중첩된 객체 타입의 JSON 요청 데이ㅣ터를 @RequestBody 어노테이션을 지정하여 중첩된 자바빈즈 매개변수로 처리한다.
